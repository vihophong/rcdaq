    ...
    if(b<100&&_linknumber!=0){
        ...
        for (int i=0;i<(int)numEvents;i++) {
            ...
            *d++=eventInfo.TriggerTimeTag;
            ...
            if (ZS_nsbl>0){ // zero suspression mode for v1730
                for (int i=0;i<MAX_N_CHANNEL;i++){
                    if (!checkpulse(Evt->DataChannel[i],Evt->ChSize[i],ZS_nsbl,WDcfg.Threshold[i],ZS_pol)){
                        Evt->ChSize[i]=8;
                    }
                }
            }
		   ...
		   ipos+=ipos_prev;
		   /*
		   if (b==0&&eventInfo.EventCounter==1000) {
